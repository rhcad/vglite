...
bool MgCommandDraw::_draw(const MgMotion* sender, GiGraphics* gs)
{
    ...
    return sender->cmds()->getSnap()->drawSnap(sender, gs) || ret;
}
...
bool MgCommandDraw::_click(const MgMotion* sender)
{
    ...
    if (shape) {
        ...
        sender->cmds()->setCommand(sender, "select");
    }
...
{
    return sender->cmds()->getActionDispatcher()->showInDrawing(sender, m_shape);
}
