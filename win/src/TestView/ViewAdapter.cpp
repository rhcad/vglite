...
void ViewAdapter::onLButtonDown(int x, int y, WPARAM wparam)
{
    _helper->onLButtonDown((float)x, (float)y,
        (wparam & MK_CONTROL) != 0, (wparam & MK_SHIFT) != 0);
}

void ViewAdapter::onLButtonUp(int x, int y)
{
    _helper->onMouseUp((float)x, (float)y);
}

void ViewAdapter::onMouseMove(int x, int y, WPARAM wparam)
{
    _helper->onMouseMove((float)x, (float)y, 
