...
class TransformCmd : public MgCommand
{
protected:
    TransformCmd();
...
// void registerTransformCmd(MgView* view)
// {
//     mgRegisterCommand<TransformCmd>(view);
...
void TransformCmd::setPointW(int index, const MgMotion* sender)
{
    ...
    if (index > 0) {
        ...
        mat = sender->view->modelTransform() * mat;
...
int TransformCmd::gatherShapes(const MgMotion* sender, MgShapes* shapes)
{
	return _lastCmd ? _lastCmd->gatherShapes(sender, shapes) : 0;
}
